...
use context_core::event::{ContextEvent, EventPayload};
...
impl EventStore {
    ...
    pub fn append(&mut self, event: ContextEvent) {
    ...
    pub fn append_batch(&mut self, events: Vec<ContextEvent>) {
        for event in events {
            self.append(event);
        }
    }
    ...
    pub fn get_by_id(&self, id: &Uuid) -> Option<&ContextEvent> {
        self.id_index.get(id).and_then(|&pos| self.events.get(pos))
    }

    /// Replay all events from the beginning (or from a timestamp).
    pub fn replay(&self, from: Option<DateTime<Utc>>) -> Vec<&ContextEvent> {
        match from {
    ...
    pub fn events_since_checkpoint(&self) -> &[ContextEvent] {
    ...
    pub fn stream(&self, stream_key: &str) -> Vec<&ContextEvent> {
        self.stream_index
            .get(stream_key)
            .map(|positions| {
                positions.iter()
    ...
    pub fn causation_chain(&self, event_id: &Uuid) -> Vec<&ContextEvent> {
...
mod tests {
    ...
    fn test_append_and_retrieve() {
        ...
        let event = ContextEvent::new(EventPayload::EntityDiscovered {
    ...
    fn test_replay() {
        let mut store = EventStore::new();
        for i in 0..5 {
            store.append(ContextEvent::new(EventPayload::EntityDiscovered {
    ...
    fn test_causation_chain() {
        ...
        let root = ContextEvent::new(EventPayload::ChangeDetected {
        ...
        let child = ContextEvent::caused_by(
