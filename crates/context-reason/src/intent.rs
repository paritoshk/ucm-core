...
mod tests {
    ...
    fn test_generate_test_intent() {
        ...
        graph.add_relationship(
            ...
            ContextEdge::new(RelationType::Imports, DiscoverySource::StaticAnalysis, 0.95, "imports"),
        ).unwrap();

        graph.add_relationship(
            &EntityId::local("src/payments/checkout.ts", "processPayment"),
            &EntityId::local("src/api/middleware.ts", "authMiddleware"),
            ContextEdge::new(RelationType::DependsOn, DiscoverySource::StaticAnalysis, 0.80, "depends"),
        ).unwrap();
