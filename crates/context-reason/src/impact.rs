...
use context_core::entity::EntityId;
use context_core::edge::ConfidenceTier;
use context_core::graph::{ContextGraph, ImpactType};
...
pub fn analyze_impact(
    ...
    // Run impact BFS
    let impacted = graph.impact_bfs(changed_entities, min_confidence, max_depth);
    let not_impacted_entities = graph.not_impacted(changed_entities, &impacted);
...
mod tests {
    ...
    fn build_test_graph() -> ContextGraph {
        ...
        for (file, symbol, name) in &entities {
            graph.add_entity(ContextEntity::new(
        ...
        graph.add_relationship(
            ...
            ContextEdge::new(RelationType::Imports, DiscoverySource::StaticAnalysis, 0.95, "imports directly"),
        ).unwrap();
        ...
        graph.add_relationship(
            ...
            &EntityId::local("src/api/middleware.ts", "authMiddleware"),
            ContextEdge::new(RelationType::DependsOn, DiscoverySource::StaticAnalysis, 0.80, "uses auth middleware"),
        ).unwrap();
