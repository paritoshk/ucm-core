...
pub fn ingest_api_logs(logs: &[ApiLogEntry]) -> Vec<ContextEvent> {
    ...
    for (key, entries) in &groups {
        ...
        if error_rate > 0.05 {
            events.push(ContextEvent::new(EventPayload::ConflictFlagged {
                entity_id: EntityId::local(&format!("api/{}", first.path), &key),
                conflict_type: context_core::event::ConflictType::RequirementDrift,
                sources: vec![context_core::event::ConflictSource {
...
pub fn ingest_api_logs_json(json: &str) -> Result<Vec<ContextEvent>, serde_json::Error> {
