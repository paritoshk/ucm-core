# ContextQA: Autonomous Impact Analysis Engine
...
### 1. Robust Rust Backend (`context-api`, `context-reason`)
...
- **Hexagonal Architecture**: Core logic (`context-reason`) is isolated from infrastructure (`context-ingest`, `context-api`).
...
## Deployment
- **Dockerized**: Multi-stage build (Rust builder image + Distroless runtime) for minimal footprint (< 50MB).
